...
export default function DoctorListingPage() {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      {/* ─── HERO ─── */}
       <div className="relative bg-gradient-to-r from-teal-700 via-teal-600 to-cyan-600 text-white pt-24 pb-16 px-4 overflow-hidden">      <div className="absolute -top-12 -right-12 w-72 h-72 bg-white/5 rounded-full pointer-events-none" />
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          <div className="inline-flex items-center gap-2 bg-white/15 backdrop-blur-sm border border-white/20 rounded-full px-4 py-1.5 mb-5">
            <UserRound className="w-4 h-4 text-teal-100" />
            <span className="text-sm font-medium text-teal-50 tracking-wide">Medical Professionals</span>
          </div>
            ...
            {!isLoading && sortedDoctors.length > 0 && (
              <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
                {sortedDoctors.map((doctor) => (
                  <DoctorCard key={doctor._id} doc={doctor} onBook={() => openModal(doctor)} />
                ))}
              </div>
            )}
